...
public final class MCheckIcons
  extends MCheckAbstract
  implements MCheckType
{
  private static final String ICONS_RULE =
    ...
    final MPackageDeclaration declaration)
  {
    ...
    if (!icons) {
      this.attribute("RuleURI", ICONS_RULE);
      this.error(
        "flatpak.icons.required",
        "Applications must define at least one (non-ICO) icon."
      );
    }
  }
}
